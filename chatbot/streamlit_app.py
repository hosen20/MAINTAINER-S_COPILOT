...
def api_request(method: str, path: str, *, token: str | None = None, json_body: dict[str, Any] | None = None, params: dict[str, Any] | None = None) -> requests.Response:
    headers = {}
    if token:
        headers["Authorization"] = f"Bearer {token}"
    return requests.request(method, f"{API_BASE_URL}{path}", headers=headers, json=json_body, params=params, timeout=60)


def login(email: str, password: str) -> None:
    response = api_request("POST", "/auth/login", json_body={"email": email, "password": password})
    if response.ok:
...
def register(email: str, password: str) -> None:
    response = api_request("POST", "/auth/register", json_body={"email": email, "password": password})
    if response.ok:
        payload = response.json()
        st.session_state.token = payload["access_token"]
        st.session_state.user = payload["user"]
...
def render_chat_page() -> None:
    ...
    with st.chat_message("user"):
        st.markdown(prompt)
    response = api_request("POST", "/chat", token=token, json_body={"message": prompt, "conversation_id": st.session_state.get("conversation_id")})
    if not response.ok:
...
def render_memory_page() -> None:
    ...
    if submitted:
        response = api_request("POST", "/chat/memory", token=token, json_body={"memory_type": memory_type, "content": content})
        if response.ok:
...
def render_widget_admin_page() -> None:
    ...
    with st.form("create-widget-form"):
        theme = st.color_picker("Primary color", value="#2563eb")
        greeting = st.text_input("Greeting", value="Hi 👋 How can I help?")
        enabled_tools_text = st.text_input("Enabled tools", value="classifier,ner,summarizer,rag,write_memory")
        allowed_origins_text = st.text_area("Allowed origins, one per line", value="http://localhost:8080\nhttp://localhost:5173")
        submitted = st.form_submit_button("Create widget")
    if submitted:
        response = api_request("POST", "/widgets", token=token, json_body={
            "theme": theme,
            "greeting": greeting,
            "enabled_tools": [x.strip() for x in enabled_tools_text.split(",") if x.strip()],
            "allowed_origins": [x.strip() for x in allowed_origins_text.splitlines() if x.strip()],
        })
        if not response.ok:
            st.error(response.text)
            return
        ...
        st.json(widget)
        snippet = f'<script src="{API_BASE_URL}/widget.js" data-widget-id="{widget["widget_id"]}" data-api-base-url="{API_BASE_URL}"></script>'
